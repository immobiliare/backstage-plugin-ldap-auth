...
import type { AuthProviderRouteHandlers, AuthResolverContext } from '@backstage/plugin-auth-node';
...
export class ProviderLdapAuthProvider implements AuthProviderRouteHandlers {
    ...
    async refresh(req: Request, res: Response): Promise<void> {
        try {
            ...
            const { profile } = await this.authHandler({ uid: result.uid as string }, ctx);

            // this sign-in the user into backstage and return an object with the token
            const backstageIdentity = await this.signInResolver({ profile, result }, ctx);
            ...
            const maxAge = Math.ceil(
                new Date(exp * 1000).valueOf() -
                    new Date().valueOf() +
                    ((this.jwtValidator as JWTTokenValidator)?.increaseTokenExpireMs || 0)
            );
